...
function er_pageinit(){
	...
	var t1=$j.template('<tr id="${uid}" ><td></td><td>${queuefriendlytime}</td> <td>${requesthost}</td>  <td class="peer pageinit">${peername}</td> <td>${scriptPreview}</td></tr>');	
...
function er_wsocketinit() {
    ...
    websocket.onmessage = function (evt) {
        ...
        if(msg.added!=null){
            var t1=$j.template('<tr id="${uid}" ><td></td><td>${queuefriendlytime}</td> <td>${requesthost}</td>  <td class="peer wsocketinit">${peername}</td> <td>${scriptPreview}</td></tr>');
