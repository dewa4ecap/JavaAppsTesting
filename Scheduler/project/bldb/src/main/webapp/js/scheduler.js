...
function scd_manageFolders(thisobj){
    ...
    dv.appendChild(bddv);
         ...
         $("scd_manageFoldersbd").appendChild(dv);
    // alert(taskid);
...
function scd_moveTasks(selobj){
    ...
    var respBack=function(data){
        if (data != null && data.tasks!=null && data.tasks.length > 0) {
            ...
            $("scd_manageFoldersbd").innerHTML = "";
            $("scd_manageFoldersbd").appendChild(dv);
            $("scd_manageFoldersbd").appendChild(dv1);
            $("scd_manageFoldersbd").appendChild(form);
...
function scd_loadSchedulePane(data){    
    ...
    if(data!=null && data.taskdata!=null & data.taskdata.timezone!=null){
        $("scd_tzInput").value=data.taskdata.timezone;
    }else{
        $("scd_tzInput").value="";
    }
    ...
    if (data != null) {
        ...
        scd_scheduleCBoxVldte($("scd_ckbox_exp_second"), null, false);
        scd_scheduleCBoxVldte($("scd_ckbox_exp_minute"), null, false);
        
        scd_scheduleCBoxVldte($("scd_ckbox_exp_hour"), null, false); // to
                                                                        // reset
                                                                        // to
                                                                        // default
        scd_scheduleCBoxVldte($("scd_ckbox_exp_hour"), null, true);
        
        scd_scheduleCBoxVldte($("scd_ckbox_exp_day"), null, false);
        scd_scheduleCBoxVldte($("scd_ckbox_exp_week"), null, false);
        scd_scheduleCBoxVldte($("scd_ckbox_exp_month"), null, false);
    }
...
function scd_parseSchedulerPane(){
    ...
    var scd_pane=$("scd_scdltable");
    for(iab=0;iab<scd_pane.rows.length;iab++){
        var hpanes=$(scd_pane.rows[iab]).select('div[type="headpane"]');
        if(hpanes.length>0){
            var ckboxs=$(hpanes[0]).select("input[type='checkbox']");           
            if(ckboxs.length>0){
                var rowid='scd_scheduler_bodyId'+iab;
                var bpanes=$(scd_pane.rows[iab]).select('div[type="bodypane"]');                
                if (bpanes.length > 0) {
                    bpanes[0].setAttribute('id',rowid);
                    
                    var tip=$(bpanes[0]).select('a[ttype="tips"]');
                    if(tip.length>0){
...
function rs_Editor(elementid,fieldid){
        ...
        $(elementid).appendChild(edp);
    
        var pr=document.createElement("pre");
        pr.className="sh_slang";                
        // pr.appendChild(document.createTextNode(container_value));
        pr.appendChild(document.createTextNode($(fieldid).value));
