...
import com.fourelementscapital.svn.SVNIExecXML;
import com.fourelementscapital.util.StringPlaceHolder4ChildStrategy;
...
public class IExecMgmt extends Authenticated{
	...
	public IExecMgmt(HttpServletRequest request,boolean ignoreSecurity) throws Exception {
	...
	public boolean moveFile2Folder(int strategy_id, int new_folder_id) throws Exception {
		...
		try{
			...
			if(new File(oldfile).renameTo(new File(newfile))){
				...
				throw new Exception("Moving failed failed...."+" Old file : "+oldfile+" ,New file : "+ newfile);
			}
