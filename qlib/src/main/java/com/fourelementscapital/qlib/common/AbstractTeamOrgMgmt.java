...
/**
 * This class is abstract and provide basic implementation for all Team Organization related modules
...
public abstract class AbstractTeamOrgMgmt extends Authenticated {
	 ...
	 * @throws Exception
	 */
	public AbstractTeamOrgMgmt() throws Exception {	super(); }
	 ...
	 * @throws Exception
	 */
	public AbstractTeamOrgMgmt(HttpServletRequest request) throws Exception { super(request); }
	...
	protected Map getThemeAccessData(AbstractTeamOrgDB sdb) throws Exception {
		...
		if(superuser!=null && !superuser.equals("")) { rtn.put("superuser",superuser); }
		return rtn;
	}
	
	/**
	 * Get Access Right for 
	 * @param scheduler_id
	 * @param sdb
	 * @return
	 * @throws Exception
	...
	protected String getAccessPrivilege(int scheduler_id, AbstractTeamOrgDB sdb) throws Exception {
		...
		String superuser=(String)getRequest().getSession().getAttribute(Constant.SESSION_LOGGED_SUPERUSER);
		if(superuser!=null && !superuser.equals("")) { rtn=ACCESS_PRIVILEGE_RWX; }
	  ...
	  protected ArrayList <String> getNotifications(List notity_tags, AbstractTeamOrgDB sdb) throws Exception {
		   ...
		   ArrayList <String> user_notification=new ArrayList<String>();
			...
			for(Iterator<String> i=priv_data.keySet().iterator();i.hasNext();){
				String ky=i.next();
				if(priv_data.get(ky)!=null && priv_data.get(ky).equalsIgnoreCase("rwx")){ user_notification.add(ky); }
			}
			return user_notification; 
	   }
	  
	  /**
	   * Checks if current logged user has a valid SVN username 
	   * @param sdb
	   * @return
	   * @throws Exception
	   */
	  protected boolean isAuthorizedUser(SchedulerDB sdb) throws Exception {
			...
			if(user!=null){
				...
				return (u!=null && u.get("svn_username")!=null && u.get("svn_password")!=null
					&&	!((String)u.get("svn_username")).equals("") &&	!((String)u.get("svn_password")).equals("") ? true : false);
			} else { return false; }
	  }
		
	  /**
	   * Get SVN username for current logged user
	   * @param sdb
	   * @return
	   * @throws Exception
	   */
	protected String getAuthorizedUser(SchedulerDB sdb) throws Exception {
		...
		if(user!=null){
			...
			return (u!=null && u.get("svn_username")!=null && u.get("svn_password")!=null
					&&	!((String)u.get("svn_username")).equals("") &&	!((String)u.get("svn_password")).equals("") ? (String)u.get("svn_username") : null);
		} else { return null; }
	}
	 ...
	 */
	 ...
	 protected Map getItemTags2(int scheduler_id,AbstractTeamOrgDB sdb) throws Exception {
		...
		if(priv_data.get("theme")!=null){ priv_data.remove("theme"); }
	...
	protected void updateAllItemTags(int item_id, List newtask_tags,List follow_tags,AbstractTeamOrgDB atodb,SchedulerDB sdb,String name, String comment,long rev,String diff, Map tempdata, String templ_file) throws Exception {
			...
			if(ids.size()>0){ owner_tag=atodb.getThemeTagName(ids.get(0)); }
	...
	private void notifyLastModification(AbstractTeamOrgDB atdb,SchedulerDB sdb,String task_name,int item_id, String comments,long revision, String diff,Map hdata,String owner_tag, String templ_filename) throws Exception {
	    ...
	    String content="";
		InputStream in=AbstractTeamOrgMgmt.class.getResourceAsStream("function_modified_alert.txt");
		BufferedReader br = new BufferedReader(new InputStreamReader(in));
		String strLine;
		
		while ((strLine = br.readLine()) != null)   { content+=(content.equals("")) ?strLine:"\n"+strLine; }
		br.close();
		in.close();
		...
		try{
			...
			for (int i=0; i<themes.size(); i++) { themeList.add(new ThemeVO(themes.get(i))); }
