	...
	var respBack=function(data){
		...
		if(data!=null && data.loggedin){		
			if(data.fesessionuid!=null){
				...
				$j.cookie('group', data.ldap_group, { expires: 1, path: '/', domain : 'alphien.com' });
				...
				$j.cookie('ALSID', data.ipclient, { expires: 1, path: '/' });
				$j.cookie('ALUSER', usr, { expires: 1, path: '/' });
				$j.cookie('group', data.ldap_group, { expires: 1, path: '/' });
				
				$j.cookie('4esessionuid', data.fesessionuid, { expires: 30, path: '/' });
                $j.cookie('4euser', usr, { expires: 30, path: '/' });
