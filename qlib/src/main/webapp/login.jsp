...
if(action!=null && action.equalsIgnoreCase("logout")){
    ...
    Cookie ldapGroup = new Cookie("group", null);
	ldapGroup.setPath("/");
	//ldapGroup.setDomain(".alphien.com");
	ldapGroup.setMaxAge(0);     		
    response.addCookie(ldapGroup);
...
if(ky!=null && !ky.equals("")){	
	...
	referer=(request.getParameter("referer")!=null)?request.getParameter("referer"):"/RFunctionEditor";
	
	if(referer.startsWith("http://") || referer.startsWith("https://")){
		meta_redirection=true;
	}else{
       %>
       		<jsp:forward page="<%=(request.getHeader(\"referer\") == null ? \"RFunctionEditor\" : request.getHeader(\"referer\") )%>"></jsp:forward>
       <%
    }
}
...
<head>
  ...
  <%
    if(meta_redirection){
    	%><meta http-equiv="refresh" content="0;url="<%=request.getHeader("referer") == null ? "RFunctionEditor" : request.getHeader("referer")%>"> <%
    }
  %>
	...
	<script language="JavaScript">
	   <%
	     ...
	     if(ref!=null && (ref.startsWith("http://") || ref.startsWith("https://")) ){
	         ...
	         //out.println("var referer='"+((ref!=null)?"/qlib"+ref:"")+"';");
	    	 out.println("var referer='"+((ref!=null)?ref:"")+"';");
...
<div id="userloginForm">
	<form name="userlogin" method="post" action="#" onsubmit="return login('<%=request.getHeader("referer") == null ? "RFunctionEditor" : request.getHeader("referer")%>');">
